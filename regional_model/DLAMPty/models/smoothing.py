...
class SegmentedSmoothing(nn.Module):
    ...
    def _in_boundary(self, i: int, j: int, x_w: int, x_h: int) -> bool:
        return (i < self.boundary_width or i >= x_w - self.boundary_width
                or j < self.boundary_width or j >= x_h - self.boundary_width)
...
class SegmentedSmoothingV2(nn.Module):
    ...
    def forward(self, x_in: torch.Tensor) -> torch.Tensor:
        ...
        # x_boundary_count = x_boundary_count.masked_fill(interior_mask, 1)  # Avoid division by zero
        x_boundary_count += 1e-6  # Avoid division by zero
